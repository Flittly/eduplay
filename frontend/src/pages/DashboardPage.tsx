...
export default function DashboardPage({ token }: DashboardPageProps) {
  ...
  const [uninstallCode, setUninstallCode] = useState<string | null>(null);
  ...
  async function handleUninstall(game: InstalledGame) {
    if (!window.confirm(`确定卸载“${game.name}”吗？卸载后本机将不再保留该游戏。`)) {
      return;
    }
    setUninstallCode(game.gameCode);
    setError("");
    setNotice("");
    try {
      await uninstallGame(token, game.gameCode);
      setGames(await listInstalledGames(token));
      setNotice(`已卸载：${game.name}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : "卸载游戏失败");
    } finally {
      setUninstallCode(null);
    }
  }
    ...
    <div className="page-content">
      ...
      {tagNames.length > 0 && (
        <div className="tag-filter-wrap">
          ...
          >
            {selectedTag ? `标签：${selectedTag}` : "标签筛选"} ▾
          </button>
          {tagMenuOpen && (
      ...
      <div className="game-grid">
        {filteredGames.length === 0 ? (
          ...
          filteredGames.map((game) => (
            <article
              key={game.gameCode}
              className="game-card game-card-with-actions"
            >
              <div className="game-card-head">
                ...
                <button
                  className="game-info-badge"
                  type="button"
                  title="查看游戏介绍与版权"
                  aria-label={`查看 ${game.name} 介绍`}
                  onClick={() => setInfoGame(game)}
                >
                  i
                </button>
              </div>
              <Link className="game-card-main" to={`/game/${game.gameCode}`}>
                  ...
                  <div className="game-tag-list">
                    {game.tags.map((tag) => (
                      <span
                        ...
                        className={`game-tag tag-${tag.category.toLowerCase()}`}
                      >
                        {tag.name}
                      </span>
                    ))}
                ...
                <span className="version">
                  已安装 v{game.installedVersion}
                </span>
              </Link>
              <button
                className="secondary full-button"
                disabled={uninstallCode === game.gameCode}
                onClick={() => void handleUninstall(game)}
              >
                {uninstallCode === game.gameCode ? "卸载中..." : "卸载游戏"}
              </button>
            </article>
          ))
        )}
      ...
      {infoGame && (
        <InfoDialog game={infoGame} onClose={() => setInfoGame(null)} />
      )}
    </div>
  );
}
